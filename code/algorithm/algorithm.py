...
class Algorithm:
    ...
    def learn(self, list_sample_data):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(f"[learn] data stacking failed: {e}")
                if list_sample_data:
                    s = list_sample_data[0]
                    self.logger.error(
                        f"[learn] sample shapes: obs={s.obs.shape if hasattr(s.obs,'shape') else type(s.obs)}, "
                        f"legal_action={s.legal_action.shape if hasattr(s.legal_action,'shape') else type(s.legal_action)}, "
                        f"prob={s.prob.shape if hasattr(s.prob,'shape') else type(s.prob)}"
                    )
            raise
